...
function Executive({ setTab }: { setTab: (t: Tab) => void }) {
  ...
  return <>
    ...
    <div className="dashboard-grid">
      ...
      <section className="card structure-card"><div className="section-head"><div><span className="section-index">02</span><strong>庫存金額結構</strong></div><span>{data.meta.latestQuarter} · 萬元</span></div>
        <div className="structure-total"><span>總庫存金額</span><strong>{fmtWan(totals.inventory)} <small>萬元</small></strong></div>
        <div className="structure-bar"><i style={{width:fmtPct(totals.inventory ? totals.net/totals.inventory : 0)}}/><em style={{width:fmtPct(totals.inventory ? (totals.wip ?? totals.inventory-totals.net)/totals.inventory : 0)}}/></div>
        <div className="structure-legend"><span><i/>淨庫存金額<b>{fmtWan(totals.net)} 萬</b></span><span><i/>WIP（擺線量）<b>{fmtWan(totals.wip ?? totals.inventory-totals.net)} 萬</b></span></div>
        <div className="ai-note"><b>管理定義</b><p>總庫存金額＝淨庫存金額＋WIP 金額。WIP 為生產線在製品，不屬於可用庫存；策略備料金額不納入庫存決策分析。</p></div>
      </section>
    </div>
...
function InventoryPage({ group }: { group: "ALL" | "FORGN" | "LP3" | "KD" }) {
  ...
  return <><div className="hero-row"><div><span className="eyebrow">{data.meta.latestQuarter} · {group}</span><h1>{group === "ALL" ? "全般庫存分析" : `${group} 庫存分析`}</h1><p>{group === "LP3" ? "庫存水準單位：日" : "庫存水準單位：月"}</p></div><div className="data-status"><span>●</span>{rows.length} 筆明細<small>依 Material Group 自動分類</small></div></div>
    {!rows.length ? <section className="card no-data"><b>{group}</b><h2>本期無 {group} 分類資料</h2><p>{data.meta.latestQuarter} 原始資料的 Material Group 未包含此分類，因此不產生推估數據。</p></section> : <>
      ...
      {group === "KD" && <section className="kd-project-grid">{[
      ...
      ].map(project=>{const p=data.kdProjects?.[project.key];return <article className={`card kd-project ${project.accent}`} key={project.key}><header><div><span>KD PROJECT</span><h2>{project.key}</h2></div><b>{project.type}</b></header>{p?<><div className="kd-project-kpis"><div><span>總庫存</span><strong>{fmtWan(p.inventory)} 萬</strong></div><div><span>淨庫存</span><strong>{fmtWan(p.net)} 萬</strong></div><div><span>件號數</span><strong>{p.count} 件</strong></div></div><p>{project.question}</p><div className="kd-project-status">{statusOrder.filter(s=>p.counts?.[s]).map(s=><span key={s}><i className={riskTone[s]}/>{s} {p.counts[s]}</span>)}</div></>:<div className="empty">本期無此專案資料</div>}</article>})}</section>}
...
function QoQPage() {
  ...
  return <><div className="hero-row"><div><span className="eyebrow">QUARTER OVER QUARTER</span><h1>QoQ 季度變化</h1><p>{data.meta.quarters.join(" → ")} 的庫存結構與生產需求趨勢</p></div></div>
    <section className="card quarter-structure-card"><div className="section-head"><div><span className="section-index">01</span><strong>季度庫存結構</strong></div><span>總庫存＝淨庫存＋WIP · 單位：萬元</span></div><div className="quarter-structure">{data.quarters.map(q=>{const wip=q.totals.wip??q.totals.inventory-q.totals.net;return <div key={q.quarter}><b>{fmtWan(q.totals.inventory)}</b><div style={{height:`${q.totals.inventory/max*100}%`}}><i style={{height:fmtPct(q.totals.inventory?q.totals.net/q.totals.inventory:0)}}/><em style={{height:fmtPct(q.totals.inventory?wip/q.totals.inventory:0)}}/></div><strong>{q.quarter}</strong></div>})}</div><div className="structure-legend centered"><span><i/>淨庫存金額</span><span><i/>WIP（擺線量）</span></div></section>
